		...
		for ((repeat = 1; repeat <= repeats; repeat++)); do
			for transport in "${transport_list[@]}"; do
				...
				taskset -c "$server_cpus" env \
					...
					wait "$server_pid"
					server_pid=
					python3 - "$work/current.json" "$transport" \
						"$client_count" "$repeat" "$start_ticks" "$end_ticks" \
						"$clock_ticks" "$wall_start" "$wall_end" "$max_rss" \
						"$work/results.tsv" "$work/results.jsonl" <<'PY'
import json
import sys

(path, transport, clients, repeat, start, end,
 clock_ticks, wall_start, wall_end, rss, tsv, jsonl) = sys.argv[1:]
...
with open(tsv, "a") as output:
    output.write(
        f"rust\t{transport}\t{clients}\t{repeat}\t"
...
print(
    f"rust {transport} clients={clients} repeat={repeat}: "
...
mkdir -p "$work/bin"
rm -f "$work/bin/porta-server" "$work/bin/loadgen" \
...
cp rust/porta-server/target/release/porta-server "$work/bin/porta-server"
