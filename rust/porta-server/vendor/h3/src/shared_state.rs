...
pub trait ConnectionState {
    ...
    /// Returns whether the peer's SETTINGS frame has been received.
    fn settings_received(&self) -> bool {
        self.shared_state().settings.get().is_some()
    }
...
mod tests {
    ...
    #[test]
    fn distinguishes_missing_settings_from_default_settings() {
        let state = SharedState::default();
        assert!(!state.settings_received());
        assert!(!state.settings().enable_datagram());
