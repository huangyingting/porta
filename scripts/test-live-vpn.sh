...
if [[ ${1:-} == --namespace ]]; then
  ...
  mkdir -p "$fake_bin" "$work/artifacts"
...
python3 - "$server" "$gateway" "$server_address" "$transports" <<'PY' || exit 2
import ipaddress
import sys
from urllib.parse import urlsplit

server, gateway, server_address, transports = sys.argv[1:]
try:
...
if not transports.split() or any(value not in ("h2", "h3") for value in transports.split()):
    raise SystemExit("PORTA_E2E_TRANSPORTS must contain h2 and/or h3")
PY
[[ ${#token} -ge 16 && $token != *[$'\r\n']* ]] || {
  echo "PORTA_E2E_TOKEN is required and must be a single line" >&2
  exit 2
}
[[ -n $identity ]] || {
...
for command in awk base64 getent grep install ip iptables nft ping python3 realpath sudo sysctl; do
...
work=$repository_root/.porta-vpn-e2e-$$
mkdir -m 0700 "$work"
cleanup() {
  status=$?
  trap - EXIT
...
if not isinstance(value, dict) or value.get("version") != 1 or not isinstance(value.get("private_key"), str):
    raise SystemExit("invalid Porta E2E identity")
key = value["private_key"]
base64.b64decode(key + "=" * (-len(key) % 4), validate=True)
PY
...
if [[ -z $client ]]; then
  (
    cd "$repository_root"
    CGO_ENABLED=0 "$go" build -trimpath -o "$work/porta-client" ./cmd/porta-client
  )
  client=$work/porta-client
fi
