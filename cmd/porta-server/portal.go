...
func (p *portalHandler) serveDownloadsPage(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodGet {
		...
		tickets := make(map[string]string, len(artifacts)+3)
		expiresAt := time.Now().Add(downloadTicketTTL)
		for _, artifact := range artifacts {
			tickets[artifact.Name] = p.issueDownloadTicket(artifact.Name, expiresAt)
		}
		for _, name := range []string{"SHA256SUMS", "SHA256SUMS.sig", "release-signing-cert.der"} {
			tickets[name] = p.issueDownloadTicket(name, expiresAt)
		}
...
func downloadsPageHTML(clientName, host, version string, artifacts []downloadArtifact, tickets map[string]string, profile *downloadProfile) string {
	...
	fmt.Fprintf(&rows, `<div class="support"><a href="/download/SHA256SUMS.sig?ticket=%s" download>Release signature</a><a href="/download/release-signing-cert.der?ticket=%s" download>Signing certificate</a></div>`,
		html.EscapeString(tickets["SHA256SUMS.sig"]), html.EscapeString(tickets["release-signing-cert.der"]))
